...
public:
   ...
   void fill(int x,int y,int iF, int id, int ex, int mem, int wb){
    ...
    int execute(vector<int> &memory,int flag,std::map<std::string, int> latencies,int cacheSize,int blockSize,int Associativity,int accessLatency,int memTime) {
        int pipeRow = 0;
...
public:
    ...
    void run(int flag1, int flag2, std::map<std::string, int> latencies,int cachesize,int blocksize,int associativity,int accesslatency,int memtime) {
        ...
        int pipeRow;
        pipeRow = cores[0].execute(memory, flag1,latencies,cachesize, blocksize,associativity, accesslatency,memtime);
        std::cout << "BUBBLE SORT: " << std::endl;
        cores[0].printval(pipeRow, flag1);
        pipeRow = cores[1].execute(memory, flag2,latencies,cachesize,blocksize, associativity, accesslatency,memtime);
